...
density_fields = {}
shear_fields   = {}
shear_maps     = {}   # raw maps kept for JK resampling
...
# ------------------------------------------------------------------ #
#  Compute workspaces ONCE on the full fields
#  Reused for all JK resamples — no workspace recomputation per resample
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
print("Building JK patches with kmeans_radec...", flush=True)

all_ra  = np.concatenate([highdens['RA'],  highlum['RA']])
all_dec = np.concatenate([highdens['DEC'], highlum['DEC']])

# Subsample for fitting — 100k is sufficient for 150 centres
...
km_centers = km.centers   # (N_JK, 2) — save for reproducibility
...
# ------------------------------------------------------------------ #
print("Assigning patch IDs...", flush=True)
...
# ------------------------------------------------------------------ #
#  JK field builders — fixed mask, signal zeroed in patch k
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
#  JK resampling loop
#  Per resample: build fields -> compute_coupled_cell -> decouple
#  with the fixed workspace. No workspace recomputation.
# ------------------------------------------------------------------ #
...
#  Sanity check
# ------------------------------------------------------------------ #
print("\nJK resample diagnostics:", flush=True)
for pair in VALID_PAIRS:
    cl      = cls_jk[pair]
    cl_std  = cl.std(axis=0)
    cl_full = pcls_ge_dict[pair][0]
    ratio   = (cl_std / np.abs(cl_full)).mean()
    print(f"  Pair {pair}: mean(std_JK)={cl_std.mean():.3e}  "
          f"mean(|Cl|)={np.abs(cl_full).mean():.3e}  ratio={ratio:.3f}")

# ------------------------------------------------------------------ #
#  JK covariance + Hartlap correction
# ------------------------------------------------------------------ #
...
# Save pseudo-Cls: columns = [lens_bin, source_bin, mode(0=E,1=B), cl_0...cl_31]
rows_ge = []
for (i, j) in VALID_PAIRS:
    for s in range(2):
